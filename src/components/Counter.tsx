...
export default function Counter() {
    ...
    const buttonCommitter=(button:typeof buttons[number]) => {
    ...
    return (
        <div className="Counter">
            <div className='view'>
                ...
                <div className={"result "+`${activeMode==='result'?'active':''}`}>{result}</div>
            </div>
            <div className="buttons">{buttons.map(i => <div className={'button ' + i.className} key={i.text} onClick={()=>buttonCommitter(i)}>{i.text}</div>)}</div>
        </div>
    )
}